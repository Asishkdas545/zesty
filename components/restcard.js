...
export default function RestCard({restInfo}){
    return (
        ...
        <div className="max-w-[280px] mb-2 transform transition duration-200 hover:scale-95 shadow-md hover:shadow-lg rounded-xl">
        ...
        <div className="w-[95%] mx-auto mt-3">
            <div className="font-bold text-xl h-7 overflow-hidden">{restInfo?.info?.name}</div>
            ...
            <div className="text-gray-600 text-xl mt-1 h-7 overflow-hidden">{restInfo?.info?.areaName }</div>
        </div>
        </div>
        </Link>
    )
